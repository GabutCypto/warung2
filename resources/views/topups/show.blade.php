                ...
                @if($topup->payment_proof)
                    <div class="mb-6">
                        ...
                        @if(in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp']))
                            <img src="{{ Storage::url($topup->payment_proof) }}" alt="Payment Proof" class="w-full max-w-md mx-auto rounded-md shadow-md">
                        @elseif($extension === 'pdf')
                            <!-- If payment proof is a PDF -->
                            <object data="{{ Storage::url($topup->payment_proof) }}" type="application/pdf" class="w-full max-w-md h-96 mx-auto border rounded-md shadow-md">
                                <p>Your browser does not support PDFs. <a href="{{ Storage::url($topup->payment_proof) }}" class="text-blue-500 hover:underline">Download the PDF</a>.</p>
                            </object>
                        @else
